...
type Stats struct {
	Turns, ToolCalls, Nudges, Repairs, RepairFailures, NewConvs, ReusedConvs, Errors, Shrinks, Replays, Synth, Primes atomic.Int64
	mu                                                                                                                sync.Mutex
	lat                                                                                                               []time.Duration
	last                                                                                                              map[string]any
}
...
func (s *Stats) handler(w http.ResponseWriter, _ *http.Request) {
	...
	writeJSON(w, 200, map[string]any{
		...
		"message_shrinks": s.Shrinks.Load(), "conversation_replays": s.Replays.Load(), "synthesized_calls": s.Synth.Load(), "primes": s.Primes.Load(), "errors": s.Errors.Load(),
